...
export class SearchTripDto {
  ...
  variantType?: string; // Optional variant filter (e.g., THEMED for Tulip)

  @IsOptional()
  @IsString()
  @IsIn(["ALL", "DOMESTIC", "INTERNATIONAL"])
  nationalityScope?: string = "ALL"; // Nationality pricing tier (DOMESTIC = WNI, INTERNATIONAL = WNA)
}
```
...
To avoid the N+1 problem and ensure optimal performance, the search query joins `product_variants` with `products`, `product_locations`, `product_trips`, and `product_trip_pricings` in a single execution — returning **one row per variant** (the unit rendered as a listing card on All Tours).
...
-- 3. Join Pricing (matching nationality scope or universal tier)
LEFT JOIN product_trip_pricings ptp ON ptp.trip_id = pt.id
    AND ptp.nationality_scope IN (:nationalityScope, 'ALL')
WHERE
    -- Strict catalog publication gate: only ACTIVE items are public
    ...
    AND pt.max_quota >= 2
GROUP BY pv.id, pv.name, pv.slug, pv.variant_type, p.id;
```

> **Listing Status Guard:** Records with `listing_status IN ('DRAFT', 'PENDING_REVIEW', 'INACTIVE', 'ARCHIVED', 'SUSPENDED')` are strictly excluded from search results at the SQL level.
>
