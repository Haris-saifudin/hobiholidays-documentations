...
| **[Area Domain Backend Guide](./area-backend-guide.md)** | Geographic Domain | `AreaModule`, `AreaService`, 4-tier tree traversal queries (`Continent → Sub Continent → Country → POI`), pure relational hierarchy, flexible flat anchoring, and in-memory reference caching. |
