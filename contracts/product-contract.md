...
> - **Read-Only Nominal Availability (Decoupled Concurrency Locking):** Catalog endpoints surface nominal available seat capacity ($\text{availableSeats} = \max(0, \text{maxQuota} - \text{bookedSeats})$). Concurrency locking and transactional quota allocation are delegated downstream to Phase 3 (Booking Domain).
