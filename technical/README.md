...
| **[Area Domain Technical Design](./area-technical-design.md)** | Geographic Hierarchy | **3-tier Geography tree** (`Continent → Country → City`, strictly capped at City level), adjacency list closure pattern, PostGIS spatial boundary models, and destination marker relations. |
...
| **[Product Media Technical Design](./product-media-technical-design.md)** | Media Asset Subsystem | **2-Phase Progressive Storage Architecture**: Phase 1 (Database-First `BYTEA` storage & streaming) to Phase 2 (Cloud S3/R2 Object Store + Cloudflare CDN), polymorphic usage binding, and database-enforced 1:1 Itinerary PDF brochure guarantee. |
